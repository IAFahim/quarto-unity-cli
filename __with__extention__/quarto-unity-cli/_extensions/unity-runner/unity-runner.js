(function () {
  ...
  function runBlock(blockId) {
    ...
    if (!sent) {
      appendOutput(blockId, "Bridge not connected. Start with: python bridge.py\n", "stderr");
  ...
  function handleExit(blockId, exitCode) {
    ...
    appendOutput(blockId, "\u2500\u2500 " + exitLabel + " \u2500\u2500\n", "system");
  ...
  function startElapsed(blockId) {
    ...
    elapsedTimers[blockId] = setInterval(function () {
  ...
  function setBlockState(blockId, state) {
    ...
    var btn = document.querySelector('.unity-run-btn[data-target="' + blockId + '"]');
    if (!btn) return;
